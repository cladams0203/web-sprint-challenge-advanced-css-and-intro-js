...
const typoFix = artists.map((item) => {
  if (item.id === 8) {
    return {
      ...item,
...
function get20s(artistArray) {
  ...
  return artistArray.filter((item) => {
    const range = item.years.split("-");
    if (range[0].includes("19")) {
      return item;
    }
  });
}
...
function addArtist(arry) {
  ...
  const newArtist = {
    ...
    bio: "Opinions are like belly buttons, everybody has one.",
  };
  const newArry = arry;
  newArry.push(newArtist);
  console.log(newArry);
}
addArtist(artists);
...
function lotsOfArt(arry) {
  /* Code here */
  const filteredArray = arry.filter((item) => item.paintings > 100);
  ...
  data.forEach((item) => {
    ...
    for (obj in item) {
      if (obj == "image") {
        ...
        tag.classList.add(obj);
        const image = document.createElement("img");
        image.src = item[obj].url;
        tag.append(image);
        ...
        link.href = item[obj].wikipedia;
        link.textContent = item[obj];
        tag.append(link);
        container.append(tag);
      } else if (obj === "bio") {
        const tag = document.createElement("div");
        tag.classList.add(obj);
        tag.textContent = item[obj];
        container.append(tag);
      }
    }
